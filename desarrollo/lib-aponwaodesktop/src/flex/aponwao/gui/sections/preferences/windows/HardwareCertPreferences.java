...
public class HardwareCertPreferences extends FieldEditorPreferencePage {
	...
	private static Logger	logger	= Logger.getLogger(HardwareCertPreferences.class.getName());
	...
	public HardwareCertPreferences() {
		...
		for (String name: PreferencesHelper.getHardwarePreferences().keySet()) {
			tempMap.put(name, PreferencesHelper.getHardwarePreferences().get(name) );
		}

		tempDefault = PreferencesHelper.getPreferences().getString(PreferencesHelper.HARDWARE_DISPOSITIVE);
                //////////////////////////////////////
	}
...
//		Map<String, String> map = PreferencesHelper.getHardwarePreferences();
//
...
//		}
//
//		ComboFieldEditor cfe = new ComboFieldEditor(PreferencesHelper.HARDWARE_DISPOSITIVE, LanguageResource.getLanguage().getString(
...
//		addField(cfe);
	...
	private void createDefaultArea() {
		...
		labelDefaultAreaTitle.setText(LanguageResource.getLanguage().getString("preferences.cert.hardware.default_title"));
		...
		labelDefaultAreaDesc.setText(LanguageResource.getLanguage().getString("preferences.cert.hardware.default_cert"));
		...
		reloadComboDefault();
		comboDefault.setText(PreferencesHelper.getPreferences().getString(PreferencesHelper.HARDWARE_DISPOSITIVE));
	...
	class ButtonAddListener implements SelectionListener {
		...
		public void widgetSelected(SelectionEvent event) {
			...
			HardwareCertUpdateDialog hardwareStoreDialog = new HardwareCertUpdateDialog(compositeMain.getShell(), tempMap);
			hardwareStoreDialog.open();
	...
	private void savePreferences() {
		...
		if (comboDefault != null) {
			...
			PreferencesHelper.saveHardwarePreferences(tempMap);
			PreferencesHelper.getPreferences().setValue(PreferencesHelper.HARDWARE_DISPOSITIVE, comboDefault.getText());
		}
	}
