...
import flex.aponwao.gui.sections.global.windows.AlertDialog;
...
public class CacheCertsPreferences extends FieldEditorPreferencePage {
	...
	private static Logger logger = Logger.getLogger(CacheCertsPreferences.class.getName());
			...
			try {
				...
				for (int i : indices) {
					...
					try {
						...
						s += LanguageResource.getLanguage().getString("preferences.trusted.show.valid.from") + " "
								+ dateFormat.format(cert.getNotBefore()) + "\n";
						s += LanguageResource.getLanguage().getString("preferences.trusted.show.valid.until") + " "
	...
	private void savePreferences() {
		...
		PreferencesHelper.setCacheKeystorePreferences(ksTemp);
	}
