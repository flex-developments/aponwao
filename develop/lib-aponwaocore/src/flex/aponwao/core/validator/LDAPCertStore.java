/*
 * @(#)LDAPCertStore.java	1.20 05/11/17
 *
...
public class LDAPCertStore extends CertStoreSpi {
     ...
     * additional criteria when the certificate's subject is different
     * than the LDAP Distinguished Name entry. The LDAPCertStore
	...
	public boolean match(Certificate cert) {
	    ...
	    boolean match = super.match(cert);
	    setSubject(subject);
	    return match;
	}

	void setCertSubject(X500Principal certSubject) {
	...
	public boolean match(CRL crl) {
	    ...
	    setIssuers(certIssuers);
	    boolean match = super.match(crl);
	    setIssuers(issuers);
	    return match;
	}

	void setCertIssuers(Collection<X500Principal> certIssuers) {
