/*
 * @(#)OCSPResponse.java	1.6 09/02/17
 *
...
class OCSPResponse {
	...
	try {
		...
		// First check if the cert matches the responder cert which was set locally.
				if (cert.equals(responderCert)) {
					...
					if (keyPurposes != null && keyPurposes.contains(KP_OCSP_SIGNING_OID)) {
						...
						boolean find = false;
						for (TrustAnchor c : params.getTrustAnchors() ) {
							
							if (cert.getSubjectX500Principal().getName().equals(c.getTrustedCert().getSubjectX500Principal().getName()))  {
	...
	public String toString() {
	    ...
	    sb.append(certId);
	    sb.append("\nCertStatus: ").append(certStatusToText(getCertStatus(null))).append("\n");
