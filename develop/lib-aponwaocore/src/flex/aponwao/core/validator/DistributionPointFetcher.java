/*
 * @(#)DistributionPointFetcher.java	1.10 06/05/08
 *
...
class DistributionPointFetcher {
	...
	(CRLSelector selector, PublicKey prevKey, String provider, 
	...
	try {
		...
		Collection<X509CRL> crls = getCRLs
		    (x509Selector, certImpl, point, 
		     reasonsMask, prevKey, provider, certStores);
		results.addAll(crls);
	    }
	    if (debug != null) {
     ...
     */
    private Collection<X509CRL> getCRLs
	(X509CRLSelector selector, X509CertImpl certImpl,
         DistributionPoint point, boolean[] reasonsMask, PublicKey prevKey, 
    ...
    private X509CRL getCRL(URIName name) {
	    ...
	    try {
		LDAPCertStore.LDAPCRLSelector sel = 
		    new LDAPCertStore.LDAPCRLSelector();
	...
	X509CRL crl, boolean[] reasonsMask, PublicKey prevKey, String provider) 
	...
	if (pointCrlIssuer != null) {
	    ...
	    boolean match = false;
            for (Iterator t = pointCrlIssuer.iterator(); 
		 !match && t.hasNext(); ) {
	...
	if (idpExt != null) {
		...
		if (point.getFullName() != null ||
		    ...
		    boolean match = false;
       	            for (Iterator i = idpNames.iterator(); 
		         !match && i.hasNext(); ) {
	        	GeneralNameInterface idpName = 
            	            ((GeneralName)i.next()).getName();
			if (debug != null) {
			    debug.println("idpName: " + idpName);
			}
       	                for (Iterator p = pointNames.iterator(); 
		             !match && p.hasNext(); ) {
	        	    GeneralNameInterface pointName = 
            	                ((GeneralName)p.next()).getName();
			    if (debug != null) {
		...
		// field of the DP
		    ...
		    boolean match = false;
            	    for (Iterator t = pointCrlIssuer.iterator(); 
			 !match && t.hasNext(); ) {
	        	GeneralNameInterface crlIssuerName = 
	            	    ((GeneralName)t.next()).getName();
            	        for (Iterator i = idpNames.iterator(); 
			     !match && i.hasNext(); ) {
	        	    GeneralNameInterface idpName = 
