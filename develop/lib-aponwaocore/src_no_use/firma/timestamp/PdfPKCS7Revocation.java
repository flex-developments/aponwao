package flex.aponwao.core.firma.timestamp;
...
//
//import flex.aponwao.core.exceptions.OCSPException;
//import flex.aponwao.core.exceptions.RevokedException;
//import flex.aponwao.core.ocsp.OCSPChecker;
//import flex.aponwao.core.ocsp.OCSPResponse;
//import flex.aponwao.core.tsa.TSResponse;
//import flex.aponwao.core.tsa.TimestampToken;
...
// */
//public class PdfPKCS7Revocation {
//
...
//
//	private static final String ID_TIME_STAMP_TOKEN = "1.2.840.113549.1.9.16.2.14"; // RFC 3161 id-aa-timeStampToken
//
//	/**
...
//	 */
//	public PdfPKCS7Revocation(byte[] contentsKey, byte[] certsKey, String provider) {
//		try {
...
//	 *            the provider or <code>null</code> for the default provider
//	 */
//	public PdfPKCS7Revocation(byte[] contentsKey, String provider) {
//
//		System.out.println("PdfPKCS7Revocation PdfPKCS7Revocation(verify) starts");
//
//		try {
...
//				System.out.println("PdfPKCS7Revocation(verify) while " + next);
//				++next;
//			}
//			System.out.println("PdfPKCS7Revocation(verify) signerInfo " + next);
...
//					}
//					
//					// Adobe revocation info
//					if (((DERObjectIdentifier) seq2.getObjectAt(0)).getId().equals(ID_ADBE_REVOCATION)) {
//						System.out.println("ID_ADBE_REVOCATION found");
//						DERSet set = (DERSet) seq2.getObjectAt(1);
//						DERSequence seq3 = (DERSequence)set.getObjectAt(0);
//						DERTaggedObject taged = (DERTaggedObject)seq3.getObjectAt(0);
//						DERSequence seq4 = (DERSequence) taged.getObjectParser(1, true);
//						DERSequence ocspRes = (DERSequence)seq4.getObjectAt(0);
//						
//						// Como no es posible hacer un cast de signCert a X509CertImpl, genero un nuevo objeto X509CertImpl.
//						ByteArrayInputStream bais = new ByteArrayInputStream (signCert.getEncoded());
//						CertificateFactory cacertf = CertificateFactory.getInstance("X.509");
//						X509CertImpl castedSignCert = (X509CertImpl) cacertf.generateCertificate(bais);
//						
//						OCSPResponse ocsp = OCSPChecker.checkOCSPResponse(ocspRes.getDEREncoded(), castedSignCert);
//						// la sentencia anterior ya valida la respuesta ocsp en si, pero hay que comprobar cosas adicionales
//						// la fecha...
//						System.out.println("OCSP validada - response fecha: " + ocsp.getProducedAtDate());
//						
//					}
//				}
//				if (digestAttr == null)
...
//				
//				if (signerInfo.getObjectAt(next) instanceof DERTaggedObject) {
//				
//					DERTaggedObject derTaggedObject = (DERTaggedObject) signerInfo.getObjectAt(next);
//		
//					if (derTaggedObject.isExplicit() && !derTaggedObject.isEmpty()) {
//		
//						DERSequence seq = (DERSequence) derTaggedObject.getObjectParser(1, true);
//						DERObjectIdentifier oi = (DERObjectIdentifier) seq.getObjectAt(0);
//						
//						if (oi.getId().equals(ID_TIME_STAMP_TOKEN)) {
//							
//							System.out.println("ID_TIME_STAMP_TOKEN found");
//							DERSet set = (DERSet) seq.getObjectAt(1);
//							DERSequence timeStampToken = (DERSequence)set.getObjectAt(0);
//							
//							// los bytes son otro pkcs7
//							PKCS7 p7 = new PKCS7(timeStampToken.getDEREncoded());
//							System.out.println("Cantidad de certificados en el timestamp: " + p7.getCertificates().length);
//							System.out.println("Primer certificado del timestamp: " + p7.getCertificates()[0].getIssuerX500Principal().getName());
//							
//							
//							// validacion del pkcs7 del timestamp
//							if (p7.getContentInfo().getContentType().equals(ContentInfo.TIMESTAMP_TOKEN_INFO_OID))
//								System.out.println("TIMESTAMP_TOKEN_INFO_OID found");
//							
//							
//							SignerInfo [] si = null;
//
...
//					        
////					        // extract the file
////					        if ((extractfile != null) && (p7.getContentInfo().getContentBytes() != null))
////					        {
////
////					           if (extractfile == System.out)
////					               // always display this banner, even if not in verbose mode (very confusing otherwise)
////					                System.err.println("====================== EXTRACTING DATA TO CONSOLE ============================");
////					            else if (verboseMode)
////					                System.err.println("======================= EXTRACTING DATA TO FILE ==============================");
////					           
////
////					            extractfile.write(p7.getContentInfo().getContentBytes());
////					            extractfile.close();        
////					        }
//
//							// fin de la validacion del pkcs7 timestamp
//					        
//					        
//					        
//							// Ahora hay que validar el timatsmptoken validacion de que la data de timestamp es correcta
//
//					        
//							// creamos el objeto TimestampToken
//							TimestampToken tst = new TimestampToken(p7.getContentInfo().getContentBytes());
//							System.out.println("Timestamp date: " + tst.getDate());
//					        
//					        // hay que comparar los datos originales
//							byte[] tsImprint = MessageDigest.getInstance("SHA-1").digest(this.digest);
//							
//							// con los que hay en el token
//							boolean blnResult = Arrays.equals(tsImprint, tst.getHashedMessage());
...
//	 *             on error
//	 */
//	public PdfPKCS7Revocation(PrivateKey privKey, Certificate[] certChain, CRL[] crlList, String hashAlgorithm, String provider,
//			boolean hasRSAdata, TSAClient tsaClient, boolean ocspAdd) throws InvalidKeyException, NoSuchProviderException,
...
//
//		if (RSAdata != null || digestAttr != null) {
//			System.out.print("digest update");
//			messageDigest.update(buf, off, len);
//		} else {
//			System.out.print("sign update!!!!!!!");
...
//     */    
//    public static Object[] verifyCertificates(Certificate chain[], KeyStore keystore, Collection crls, Calendar calendar) {
//    	
//    	// estaria bien que se comprobara el keyusage de los certs
//        
//    	if (calendar == null)
//            calendar = new GregorianCalendar();
//    	
...
//	 *            the encryption algorithm. It may must be <CODE>null</CODE> if the <CODE>digest</CODE> is also <CODE>null</CODE>. If the
//	 *            <CODE>digest</CODE> is not <CODE>null</CODE> then it may be "RSA" or "DSA"
//	 */
...
//
//	/**
//	 * Gets the bytes for the PKCS7SignedData object. Optionally the authenticatedAttributes in the signerInfo can also be set. If either of
...
//				ASN1EncodableVector attribute = buildAuthenticatedAttributes(secondDigest, signingTime);
...
////						// System.out.println(new DERTaggedObject(false, 1, new DERSet(unauthAttributes)).toString());
...
//
//			// TODO esto es necesario? si adobe no lo tiene en cuenta no merece la pena volverlo a meter.
//			if (!crls.isEmpty()) {
//				v = new ASN1EncodableVector();
...
//
//	/**
//	 * When using authenticatedAttributes the authentication process is different. The document digest is generated and put inside the
//	 * attribute. The signing is done over the DER encoded authenticatedAttributes. This method provides that encoding and the parameters
//	 * must be exactly the same as in {@link #getEncodedPKCS7(byte[],Calendar)}.
//	 * <p>
//	 * A simple example:
//	 * <p>
//	 * 
...
//	 * @throws CRLException
//	 */
//	public byte[] getAuthenticatedAttributeBytes(byte secondDigest[], Calendar signingTime) throws CRLException, IOException,
//			OCSPException {
//
//		if (this.ocsp)
//			this.ocspResponse = OCSPChecker.check((X509CertImpl) signCert);
//
//		ASN1EncodableVector attribute = buildAuthenticatedAttributes(secondDigest, signingTime);
//		ByteArrayOutputStream bOut = new ByteArrayOutputStream();
//		ASN1OutputStream dout = new ASN1OutputStream(bOut);
...
//	 * @throws OCSPException
//	 */
//	private ASN1EncodableVector buildAuthenticatedAttributes(byte secondDigest[], Calendar signingTime) throws IOException, CRLException,
//			OCSPException {
//
//		ASN1EncodableVector attribute = new ASN1EncodableVector();
//		ASN1EncodableVector v = new ASN1EncodableVector();
...
//
//			if (this.ocsp) {
//				ASN1EncodableVector v12 = new ASN1EncodableVector();
//				
//				// grabo a fichero los bytes para comprobar que son lo mismo que los que se guardan al firmar.
////				FileOutputStream fos = new FileOutputStream("/home/alfredo/Escritorio/ocsp1.txt");
////				fos.write(this.ocspResponse.getBytes());
//				
//				ASN1InputStream t3 = new ASN1InputStream(new ByteArrayInputStream(this.ocspResponse.getBytes()));
//				v12.add(t3.readObject());
//				v1.add(new DERTaggedObject(true, 1, new DERSequence(v12)));
//			}
//
//			v.add(new DERSet(new DERSequence(v1)));
//
//			attribute.add(new DERSequence(v));
//		}
//
...
//			return null;
//
//		FileOutputStream fos = new FileOutputStream("/home/alfredo/Escritorio/timestamp1.txt");
//		fos.write(timeStampToken);
//
...
//
//	/**
//	 * class for breaking up an X500 Name into it's component tokens, ala java.util.StringTokenizer. We need this class as some of the
//	 * lightweight Java environment don't support classes like StringTokenizer.
//	 */
